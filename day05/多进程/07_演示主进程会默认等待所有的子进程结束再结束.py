...
def study():
    for i in range(20):
        print(f'{multiprocessing.current_process().name}子进程任务执行中...{i}')
        time.sleep(0.1)
    print(f'{multiprocessing.current_process().name}主进程任务结束...')


# 2. 再调用任务
if __name__ == '__main__':
    print(f'{multiprocessing.current_process().name}主进程任务执行中...')
    # 创建子进程
